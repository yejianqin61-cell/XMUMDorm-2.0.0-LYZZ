...
export default function CampusPostDetailScreen({ postId, onBack }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      apiGet(`/api/square/campus-posts/${postId}`),
      apiGet(`/api/square/campus-posts/${postId}/comments`),
    ]).then(([p, c]) => {
      if (p.status === 0) {
        setPost(p.data);
  ...
  const handleLike = async () => {
    ...
    try {
      const data = await apiPost(`/api/square/campus-posts/${postId}/like`);
      if (data.status === 0) {
        setLiked(data.liked);
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      const data = await apiPost(`/api/square/campus-posts/${postId}/comments`, body);
      if (data.status === 0) {
        setCommentText('');
        setReplyingTo(null);
        const c = await apiGet(`/api/square/campus-posts/${postId}/comments`);
        if (c.status === 0) setComments(c.data || []);
      } else {
  ...
  const feedLabel = post?.feed_tab === 'college' ? '学院通知' : '学校公告';

  return (
    <SafeAreaView style={s.bg} edges={['top']}>
      <View style={s.header}>
        <Pressable onPress={onBack}><Text style={s.back}>← 广场</Text></Pressable>
        <Text style={s.headerTitle}>{feedLabel}</Text>
        <View style={{ width: 50 }} />
      </View>
        ...
        {post?.organization && (
          <View style={s.orgBanner}>
            {post.organization.avatar ? (
              <Image source={{ uri: prefixImg(post.organization.avatar) }} style={s.orgAvatar} />
            ) : (
              <View style={[s.orgAvatar, s.orgAvatarPlace]}><Text>🏫</Text></View>
            )}
            <View style={{ flex: 1 }}>
              <Text style={s.orgName}>{post.organization.name}</Text>
              <Text style={s.orgBadge}>✓ 官方认证</Text>
            </View>
          </View>
        )}
...
const s = StyleSheet.create({
  ...
  orgBanner: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#fff', borderRadius: 14, padding: 14, marginBottom: 12 },
  orgAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#f1f5f9' },
  orgAvatarPlace: { justifyContent: 'center', alignItems: 'center' },
  orgName: { fontSize: 15, fontWeight: '700', color: '#0f172a' },
  orgBadge: { fontSize: 11, color: '#2563eb', marginTop: 2 },
