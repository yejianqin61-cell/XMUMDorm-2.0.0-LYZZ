/**
 * 执行 019：course_reviews.tag
 *
 * 用法：
 *   node scripts/run-migration-019-course-reviews-tag.js
 */
const fs = require('fs');
...
async function run() {
  ...
  try {
    ...
    const file = path.join(ROOT, 'migrations', '019_course_reviews_tag.sql');
    if (!fs.existsSync(file)) {
      console.error('迁移文件不存在：', file);
      process.exit(1);
    }
    console.log('\n=== 正在执行迁移：019_course_reviews_tag.sql ===');
    let sql = fs.readFileSync(file, 'utf8');
    sql = sql.replace(/^\s*USE\s+jack_campus\s*;?\s*$/gim, '');

    try {
      await conn.query(sql);
      console.log('迁移 019 执行完成。');
    } catch (err) {
