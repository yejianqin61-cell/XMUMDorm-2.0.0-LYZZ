/**
 * 一键执行 009_timetable_import.sql（建课程表相关表）
 *
 * 用法（项目根目录）：
 *   node scripts/migrate_009_timetable_import.js
 *
 * 连接配置沿用项目约定：
...
async function main() {
  const sqlPath = path.join(__dirname, '..', 'migrations', '009_timetable_import.sql');
  ...
  try {
    console.log('✅ MySQL 已连接，开始执行 009_timetable_import.sql ...');
    await conn.query(sql);
    console.log('🎉 执行完成：课程表相关表已创建/更新。');
  } catch (e) {
