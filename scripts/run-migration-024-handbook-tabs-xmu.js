/**
 * 执行 Handbook Tabs 迁移（024）
 *
 * 用法：
 *   node scripts/run-migration-024-handbook-tabs-xmu.js
 *
 * 依赖：
...
async function run() {
  ...
  try {
    ...
    const file = path.join(ROOT, 'migrations', '024_handbook_tabs_xmu.sql');
    if (!fs.existsSync(file)) {
      console.error('迁移文件不存在：', file);
      process.exit(1);
    }

    console.log('\n=== 正在执行迁移：024_handbook_tabs_xmu.sql ===');
    let sql = fs.readFileSync(file, 'utf8');
    sql = sql.replace(/^\s*USE\s+jack_campus\s*;?\s*$/gim, '');

    await conn.query(sql);
    console.log('迁移 024_handbook_tabs_xmu.sql 执行完成。');
    console.log('\n完成。');
    process.exit(0);
  } catch (err) {
