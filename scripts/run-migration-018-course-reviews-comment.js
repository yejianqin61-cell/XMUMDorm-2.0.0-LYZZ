/**
 * 执行 018：course_reviews.comment
 *
 * 用法：
 *   node scripts/run-migration-018-course-reviews-comment.js
 */
const fs = require('fs');
...
async function run() {
  ...
  try {
    ...
    const file = path.join(ROOT, 'migrations', '018_course_reviews_comment.sql');
    if (!fs.existsSync(file)) {
      console.error('迁移文件不存在：', file);
      process.exit(1);
    }
    console.log('\n=== 正在执行迁移：018_course_reviews_comment.sql ===');
    let sql = fs.readFileSync(file, 'utf8');
    sql = sql.replace(/^\s*USE\s+jack_campus\s*;?\s*$/gim, '');

    try {
      await conn.query(sql);
      console.log('迁移 018 执行完成。');
    } catch (err) {
