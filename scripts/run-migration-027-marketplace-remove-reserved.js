/**
 * 执行 Marketplace 状态迁移（027）：移除 reserved
 *
 * 用法：
 *   node scripts/run-migration-027-marketplace-remove-reserved.js
 */
const fs = require('fs');
...
async function run() {
  ...
  try {
    ...
    const file = path.join(ROOT, 'migrations', '027_marketplace_remove_reserved.sql');
    if (!fs.existsSync(file)) {
      console.error('迁移文件不存在：', file);
      process.exit(1);
    }

    console.log('\n=== 正在执行迁移：027_marketplace_remove_reserved.sql ===');
    let sql = fs.readFileSync(file, 'utf8');
    sql = sql.replace(/^\s*USE\s+jack_campus\s*;?\s*$/gim, '');
    await conn.query(sql);
    console.log('迁移 027_marketplace_remove_reserved.sql 执行完成。');
    process.exit(0);
  } catch (err) {
