/**
 * 执行 Course Reviews 学期迁移（023）
 *
 * 用法：
 *   node scripts/run-migration-023-course-reviews-term.js
 *
 * 依赖：
...
async function run() {
  ...
  try {
    ...
    const file = path.join(ROOT, 'migrations', '023_course_reviews_term.sql');
    if (!fs.existsSync(file)) {
      console.error('迁移文件不存在：', file);
      process.exit(1);
    }

    console.log('\n=== 正在执行迁移：023_course_reviews_term.sql ===');
    let sql = fs.readFileSync(file, 'utf8');
    sql = sql.replace(/^\s*USE\s+jack_campus\s*;?\s*$/gim, '');

    try {
      await conn.query(sql);
      console.log('迁移 023_course_reviews_term.sql 执行完成。');
    } catch (err) {
