/**
 * 执行 Handbook Tabs 排序迁移（025）
 *
 * 用法：
 *   node scripts/run-migration-025-handbook-tabs-order.js
 */
const fs = require('fs');
...
async function run() {
  ...
  try {
    ...
    const file = path.join(ROOT, 'migrations', '025_handbook_tabs_order.sql');
    if (!fs.existsSync(file)) {
      console.error('迁移文件不存在：', file);
      process.exit(1);
    }

    console.log('\n=== 正在执行迁移：025_handbook_tabs_order.sql ===');
    let sql = fs.readFileSync(file, 'utf8');
    sql = sql.replace(/^\s*USE\s+jack_campus\s*;?\s*$/gim, '');
    await conn.query(sql);
    console.log('迁移 025_handbook_tabs_order.sql 执行完成。');
    process.exit(0);
  } catch (err) {
