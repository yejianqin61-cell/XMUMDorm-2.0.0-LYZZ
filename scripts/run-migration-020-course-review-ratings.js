/**
 * 执行 020：course_review_ratings
 *
 * 用法：
 *   node scripts/run-migration-020-course-review-ratings.js
 */
const fs = require('fs');
...
async function run() {
  ...
  try {
    ...
    const file = path.join(ROOT, 'migrations', '020_course_review_ratings.sql');
    if (!fs.existsSync(file)) {
      console.error('迁移文件不存在：', file);
      process.exit(1);
    }
    console.log('\n=== 正在执行迁移：020_course_review_ratings.sql ===');
    let sql = fs.readFileSync(file, 'utf8');
    sql = sql.replace(/^\s*USE\s+jack_campus\s*;?\s*$/gim, '');

    await conn.query(sql);
    console.log('迁移 020 执行完成。');
    process.exit(0);
  } catch (err) {
