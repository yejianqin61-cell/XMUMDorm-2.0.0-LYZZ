/**
 * 执行 021：course_reviews.tags_json（多选标签）
 *
 * 用法：
 *   node scripts/run-migration-021-course-reviews-tags-json.js
 */
const fs = require('fs');
...
async function run() {
  ...
  try {
    ...
    console.error('迁移失败：', err && (err.stack || err.message || err));
    process.exit(1);
  } finally {
